...
function Login(props) {
    ...
    const loginCreate = () => {
        console.log('loginInitiate calling.........', navigator);
        navigator.credentials.create(createCredentialDefaultArgs)
            .then((cred) => {
                console.log("NEW CREDENTIAL", cred);
                //setRawId(cred.rawId);
    ...
    const loginGet = (rawId) => {
        ...
        navigator.credentials.get(getCredentialDefaultArgs)
            .then((cred) => {
                navigate('/home');
                console.log("succesfully logged in", cred);
            })
            .catch((err) => {
                console.log("loginGet ERROR", err);
            });
    }
    useEffect(() => {
        //loginCreate();
    ...
    return (
        <div className="Login">
            <h1>Login</h1>
            <button onClick={() => loginGet()}>cliccckk..</button>
        </div>
    );
}
